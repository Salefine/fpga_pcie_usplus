...
// SG_DMA 单次最大传输 = SG_NUM_MAX × PAGE_SIZE = 512 × 4KB = 2MB
#define SG_DMA_CHUNK_SIZE (512u * 4096u)
...
int main(int argc, char **argv)
{
    ...
    size_t aligned_size = (total_size + 4095) & ~4095ULL;
    chunk_buf = (unsigned char *)aligned_alloc(4096, aligned_size);
    if (!chunk_buf) {
    ...
    int ret = fpga_sg_c2h(fpga, (unsigned int *)chunk_buf, total_size, (long long)timeout_ms, fpga_addr);
    
    if (ret < 0 || (unsigned int)ret != total_size) {
        fprintf(stderr,"DMA failed at offset , ret=%d\n",ret);
        free(chunk_buf);
        fclose(fp);
        fpga_close(fpga);
        return -1;
    ...
    if(fwrite(chunk_buf, 1, total_size, fp) != total_size){
        perror("fwrite");
        free(chunk_buf);
        fclose(fp);
        fpga_close(fpga);
